"""Batch generation: turn one new podcast episode into a set of Shorts.

`generate_batch` does the whole find-episode -> pick-moments -> clip flow and
returns the clips it produced (with a title + description ready for upload).
It is reused by both the manual Start button (Pipeline) and the autopilot.
"""
...
# Playback speed applied to the top clip for every episode (1.0 = normal).
CLIP_SPEED = 1.3
...
def _hashtag(name: str) -> str:
    tag = re.sub(r"[^A-Za-z0-9]", "", name or "")
    return f"#{tag}" if tag else ""


def _build_description(title: str, vinfo) -> str:
    tags = " ".join(t for t in ["#Shorts", "#podcast", _hashtag(vinfo.channel)] if t)
    return (
        f"{title}\n\n"
        f"Clip from “{vinfo.title}” — {vinfo.channel}\n"
        f"Source: {vinfo.url}\n\n{tags}"
    )


def generate_batch(s: Settings, st: JobState) -> List[dict]:
    """Produce Shorts from one new episode. Returns a list of clip dicts:
    {path, title, description, source_title, source_url}. Raises on hard errors.
    """
    with _GEN_LOCK:
        ...
        # First NEW episode with a transcript that also downloads (3s probe).
        ...
        for vid in ids:
            ...
            if attempts > MAX_SCAN_ATTEMPTS:
                st.log.append(f"  gave up after inspecting {MAX_SCAN_ATTEMPTS} episodes.")
                break
        ...
        if not vinfo or not chosen_segments:
            raise RuntimeError(
                ...
                "(no captions / not downloadable / already used)."
            )

        # Find all good moments (one OpenAI call; heuristic fallback).
        picked = []
        if s.openai_api_key:
        ...
        speed = CLIP_SPEED
        if speed != 1.0:
            st.log.append(f"  speeding clips {speed}x.")
        made: List[dict] = []
        total = len(picked)
        for n, moment in enumerate(picked, start=1):
            ...
            title = (moment.title or vinfo.title)[:100]
            description = _build_description(title, vinfo)
...
class Pipeline:
    """Manual single-batch generation behind the Start button (no uploading)."""
    ...
    def _run(self) -> None:
        st = self.state
        try:
            made = generate_batch(self.settings, st)
            st.set("done", f"Done: made {len(made)} short(s).")
        except Exception as exc:  # noqa: BLE001
            st.error = str(exc)
